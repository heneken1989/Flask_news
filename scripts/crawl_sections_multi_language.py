...
SECTION_URLS = {
    'erhverv': {
        ...
        'kl': 'https://kl.sermitsiaq.ag/tag/inuussutissarsiutit'
    },
    'samfund': {
        'da': 'https://www.sermitsiaq.ag/tag/samfund',
        'kl': 'https://kl.sermitsiaq.ag/tag/inuiaqatigiit'
    },
    'kultur': {
        'da': 'https://www.sermitsiaq.ag/tag/kultur',
        'kl': 'https://kl.sermitsiaq.ag/tag/kulturi'
    },
    'sport': {
        'da': 'https://www.sermitsiaq.ag/tag/sport',
        'kl': 'https://kl.sermitsiaq.ag/tag/timersorneq'
    },
    'podcasti': {
        'da': 'https://www.sermitsiaq.ag/podcasti',
        'kl': 'https://kl.sermitsiaq.ag/podcasti-kl'
    }
}
...
def translate_dk_section_to_en(section_name):
    ...
    # ⚠️ COMMENT: Check các DA articles chưa có EN version - KHÔNG CẦN THIẾT
    # Lý do: translate_articles_batch() ở trên đã check và skip articles đã có EN version
    # Logic này duplicate và làm chậm process
...
def main():
    ...
    with app.app_context():
        ...
        # ⚠️ CHANGED: Không dùng process_home() nữa, sẽ dùng link_home_articles.py ở cuối
        if args.section == 'home':
            print("\n" + "="*80)
            print("🏠 HOME PROCESSING")
            print("="*80)
        ...
        # link_home_articles.py sẽ:
        # 1. Crawl home layout (DA và KL)
        # 2. Link articles với layout (update metadata)
        # 3. Match DA ↔ KL (set canonical_id)
        # 4. Create missing EN articles
        # 5. Generate sitemaps
        print("\n" + "="*80)
        ...
        try:
            ...
            if result.returncode == 0:
                print("\n" + "="*80)
                print("✅ Home articles processing completed!")
                print("="*80)
            else:
