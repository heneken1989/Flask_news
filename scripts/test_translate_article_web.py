"""
Script để test dịch article detail bằng Google Translate Web và tạo EN version mới với URL có đuôi "-test"

Usage:
    # Test với article ID
    python flask/scripts/test_translate_article_web.py --article-id 1853
    
    # Test với URL
    python flask/scripts/test_translate_article_web.py --url "https://www.sermitsiaq.ag/..."
    
    # Chạy với browser visible (để debug)
    python flask/scripts/test_translate_article_web.py --article-id 1853 --no-headless
"""
import sys
import os
import argparse
...
# Google account credentials (optional - để login và tăng giới hạn dịch)
...
def ensure_google_login(sb):
    """
    Đảm bảo đã login vào Google account (nếu có credentials)
    Login vào Google có thể tăng giới hạn dịch của Google Translate Web
    
    Args:
        sb: SeleniumBase instance
    
    Returns:
        True nếu đã login hoặc không cần login, False nếu login thất bại
    """
    # Nếu không có credentials, bỏ qua
    if not GOOGLE_EMAIL or not GOOGLE_PASSWORD:
        print("   ℹ️  No Google credentials provided, using without login (may have lower limits)")
        return True
    
    try:
        print("   🔐 Checking Google login status...")
        
        # Mở Google account page để kiểm tra
        sb.open("https://accounts.google.com/")
        sb.sleep(2)
        
        # Kiểm tra xem đã login chưa
        current_url = sb.get_current_url()
        if 'accounts.google.com/ServiceLogin' not in current_url and 'myaccount.google.com' in current_url:
            print("   ✅ Already logged in to Google")
            return True
        
        # Nếu chưa login, thử login
        print("   🔑 Logging in to Google account...")
        
        # Tìm và điền email
        try:
            email_input = sb.find_element('input[type="email"]', timeout=5)
            if email_input:
                sb.type('input[type="email"]', GOOGLE_EMAIL)
                sb.sleep(1)
                
                # Click Next
                next_button = sb.find_element('button:contains("Next")', timeout=3)
                if next_button:
            ...
            # Thử selector khác
            try:
                sb.type('#identifierId', GOOGLE_EMAIL)
                sb.sleep(1)
        ...
        try:
            ...
            if password_input:
                ...
                # Click Next/Sign in
                signin_button = sb.find_element('button:contains("Next")', timeout=3)
                if signin_button:
                    sb.click('button:contains("Next")')
                    sb.sleep(3)
        except:
            # Thử selector khác
            try:
                sb.type('input[name="password"]', GOOGLE_PASSWORD)
                sb.sleep(1)
                sb.click('#passwordNext')
                sb.sleep(3)
            except Exception as e:
                print(f"   ⚠️  Could not find password input: {e}")
                return False
        ...
        if 'myaccount.google.com' in current_url or 'accounts.google.com' not in current_url:
            ...
            print("   ⚠️  Login may have failed, but continuing anyway...")
            return True  # Vẫn tiếp tục dù có thể chưa login thành công
            
    except Exception as e:
        print(f"   ⚠️  Error during Google login: {e}")
        print("   ⚠️  Continuing without login...")
        return True  # Vẫn tiếp tục dù login thất bại


@contextmanager
def start_browser_for_translate(headless=True, login_google=False, manual_login=False):
    """
    Start browser để sử dụng Google Translate Web
    
    Args:
        headless: Run browser in headless mode
        login_google: Login vào Google account nếu True (có thể tăng giới hạn dịch)
        manual_login: Nếu True, đợi user đăng nhập thủ công
    ...
    try:
        ...
        if login_google:
            if manual_login:
                ...
                print("   🔐 Please login to Google account manually in the browser...")
                print("   ⏳ Waiting for you to complete login...")
                print("   💡 After logging in, press Enter in the terminal to continue...")
                
                # Mở Google account page để user có thể login
                sb.open("https://accounts.google.com/")
                sb.sleep(2)
                
                # Đợi user nhấn Enter
                # Auto-continue after manual login (no pause needed)
                sb.sleep(2)  # Đợi một chút để đảm bảo login hoàn tất
                ...
                if 'myaccount.google.com' in current_url or 'accounts.google.com' not in current_url:
                    ...
                    print("   ⚠️  May not be logged in, but continuing anyway...")
            else:
                # Tự động login
                ensure_google_login(sb)
                    ...
                    while wait_time < max_wait:
                        try:
                            ...
                            if main_result_parts:
                                ...
                                # Gộp tất cả parts, giữ nguyên separators
                            ...
                            # Cách 2: Thử lấy từ các elements trực tiếp (fallback)
                            if not translated_text or len(translated_text) < len(text_to_translate) * 0.3:
                                result_selectors = [
                                ...
                                all_elements_text = []
                                # Với text ngắn, điều chỉnh threshold
                                min_elem_length = 1 if len(text_to_translate) < 50 else 5
                                
                                for selector in result_selectors:
                                    try:
                                        result_elems = sb.find_elements(selector, timeout=1)
                                        for elem in result_elems:
                                            elem_text = elem.text.strip()
                                            if elem_text and len(elem_text) >= min_elem_length:
                                                if elem_text not in all_elements_text:
                                                    all_elements_text.append(elem_text)
                                    except:
                                        continue
                                
                                if all_elements_text:
                                    combined_text = ' '.join(all_elements_text)
                                    combined_text = ' '.join(combined_text.split())
                                    if combined_text and len(combined_text) > len(translated_text or ''):
                                        translated_text = combined_text
                                        print(f"      ✅ Found {len(all_elements_text)} elements, combined: {len(translated_text)} chars")
...
def create_test_en_url(da_url):
    """
    Tạo test EN URL từ DA URL bằng cách thêm "-test" vào cuối
    ...
    Returns:
        Test EN URL
    """
    # Convert DA URL sang EN URL format
    # Ví dụ: https://www.sermitsiaq.ag/.../2329146 -> https://www.sermitsiaq.ag/.../2329146-test
    if da_url.endswith('/'):
        test_url = da_url.rstrip('/') + '-test'
    else:
        test_url = da_url + '-test'
    return test_url


def test_translate_article_detail(article_id=None, article_url=None, headless=True, no_google_login=False, manual_login=False):
    """
    Test dịch article detail bằng Google Translate Web và tạo EN version mới với URL có đuôi "-test"
    ...
    with app.app_context():
        ...
        print(f"\n{'='*60}")
        print(f"📰 Testing Article Detail Translation with Google Translate Web")
        print(f"{'='*60}")
        ...
        if article_detail.language != 'da':
            print(f"⚠️  Article detail is not in Danish (language: {article_detail.language})")
            print(f"   This script is designed to translate from DA to EN")
            return
        ...
        print(f"📝 Found {len(content_blocks)} content blocks to translate\n")
        
        # Tạo test EN URL
        da_url = article_detail.published_url
        test_en_url = create_test_en_url(da_url)
        
        print(f"🔗 Test EN URL: {test_en_url}\n")
        
        # Kiểm tra xem đã có test EN version chưa - tự động xóa để tạo lại
        existing_test = ArticleDetail.query.filter_by(published_url=test_en_url, language='en').first()
        if existing_test:
            print(f"⚠️  Test EN ArticleDetail already exists:")
            print(f"   ID: {existing_test.id}")
            print(f"   URL: {existing_test.published_url}")
            ...
            print(f"   Created: {existing_test.created_at}")
            print(f"   Updated: {existing_test.updated_at}")
            print(f"\n   🗑️  Deleting existing test ArticleDetail to recreate...")
            try:
                db.session.delete(existing_test)
                db.session.commit()
                print(f"      ✅ Successfully deleted test ArticleDetail ID {existing_test.id}")
                print(f"      ✅ Ready to create new test version\n")
            except Exception as e:
                print(f"      ❌ Error deleting test ArticleDetail: {e}")
                db.session.rollback()
                print(f"      ⚠️  Will try to continue anyway...\n")
        else:
            print(f"✅ No existing test EN version found, will create new one\n")
        
        # Dịch content blocks bằng Google Translate Web
        print(f"🌐 Starting translation with Google Translate Web...")
        print(f"   This may take a while depending on the number of blocks...\n")
        
        start_time = time.time()
        
        # Kiểm tra xem có nên login Google không
        if manual_login:
            login_google = True
            # Manual login cần browser visible
            if headless:
                print(f"   ⚠️  Manual login requires visible browser, disabling headless mode")
                headless = False
            print(f"   🔐 Manual Google login enabled")
            print(f"   💡 Browser will open, please login to Google manually, then press Enter in terminal")
        else:
            login_google = bool(GOOGLE_EMAIL and GOOGLE_PASSWORD) and not no_google_login
            if login_google:
                print(f"   🔐 Google credentials found, will auto-login to increase translation limits")
            else:
                if no_google_login:
                    print(f"   ℹ️  Google login disabled by --no-google-login flag")
                else:
                    print(f"   ℹ️  No Google credentials, using without login (may have lower limits)")
                    print(f"   💡 Tip: Use --manual-login to login manually, or set GOOGLE_EMAIL and GOOGLE_PASSWORD in .env")
        
        with start_browser_for_translate(headless=headless, login_google=login_google, manual_login=manual_login) as sb:
            translated_blocks = translate_content_blocks_with_web(
                sb,
                content_blocks,
                source_lang='da',
                target_lang='en',
                delay=2.0,  # Delay 2 giây giữa các lần dịch
                batch_size=5  # Gom 5 blocks một lần để ổn định hơn
            )
        ...
        # Tạo ArticleDetail mới với language='en' và test URL
        test_en_article_detail = ArticleDetail(
            published_url=test_en_url,
        ...
        try:
            db.session.add(test_en_article_detail)
            db.session.commit()
            print(f"✅ Successfully created test EN article detail!")
            print(f"   ArticleDetail ID: {test_en_article_detail.id}")
            print(f"   Published URL: {test_en_article_detail.published_url}")
            print(f"   Language: {test_en_article_detail.language}")
            print(f"   Blocks: {len(test_en_article_detail.content_blocks)}")
            print(f"\n📝 Test EN URL: {test_en_url}")
            print(f"📝 Original DA URL: {da_url}")
            print(f"\n🌐 View test article detail:")
            print(f"   http://127.0.0.1:5000/article/detail/test?url={test_en_url}&lang=en")
            
            # Kiểm tra xem có EN version cũ không
            original_en_url = da_url.replace('kl.sermitsiaq.ag', 'www.sermitsiaq.ag')
            original_en = ArticleDetail.query.filter_by(published_url=original_en_url, language='en').first()
            if original_en:
                print(f"\nℹ️  Original EN version still exists:")
                print(f"   ID: {original_en.id}")
                print(f"   URL: {original_en.published_url}")
                print(f"   Blocks: {len(original_en.content_blocks) if original_en.content_blocks else 0}")
            
        except Exception as e:
            db.session.rollback()
            print(f"❌ Error creating test EN article detail: {e}")
            import traceback
            traceback.print_exc()
...
def main():
    parser = argparse.ArgumentParser(
        description='Test translate article detail with Google Translate Web and create test EN version',
    ...
    parser.add_argument('--no-google-login', action='store_true',
                        help='Disable Google login even if credentials are available')
    parser.add_argument('--manual-login', action='store_true',
                        help='Enable manual Google login (browser will open, wait for you to login, then press Enter)')
    
    args = parser.parse_args()
    
    if not args.article_id and not args.url:
        parser.print_help()
        return
    
    test_translate_article_detail(
        article_id=args.article_id,
        article_url=args.url,
        headless=not args.no_headless,
        no_google_login=args.no_google_login,
