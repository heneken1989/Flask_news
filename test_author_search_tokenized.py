#!/usr/bin/env python3
"""
Test author search with tokenized search
Ensure author names in ArticleDetail.content_blocks are searchable
"""
...
def test_author_search(query, language='da', limit=3):
    """Test that author names are searchable"""
    with app.app_context():
        print(f"\n{'='*80}")
        print(f"👤 Testing Author Search (Tokenized)")
        print(f"{'='*80}")
        print(f"Query: '{query}'")
        ...
        # Build OR conditions for each word (including author search)
        word_conditions = []
        for word in words:
            word_pattern = f"%{word}%"
        ...
        if results:
            for idx, (article, article_detail, relevance) in enumerate(results, 1):
                ...
                if article_detail and article_detail.content_blocks:
                    try:
                        blocks = article_detail.content_blocks
                        ...
                        if isinstance(blocks, list) and len(blocks) > 0:
                            ...
                            if 'bylines' in first_block:
                                for byline in first_block['bylines']:
                                    if 'fullname' in byline:
                                        authors.append(byline['fullname'])
                                    if 'description' in byline:
                                        authors.append(f"({byline['description']})")
                        
                        if authors:
                            print(f"   👤 Authors: {' '.join(authors)}")
                        else:
                            print(f"   👤 Authors: (no bylines found)")
                    except Exception as e:
                        print(f"   👤 Authors: (error parsing: {e})")
                else:
                    print(f"   👤 Authors: (no article_detail)")
                ...
                for word in words:
                    if word.lower() in article.title.lower():
                        ...
                        if word.lower() in str(article_detail.content_blocks).lower():
                            matched_words.append(f"{word}(AUTHOR)")
                
                print(f"   ✅ Matched: {', '.join(matched_words)}")
                print(f"")
        else:
...
if __name__ == '__main__':
    # Test 1: Full author name
    test_author_search("Jens Frederik Nielsen", language='da')
    
    # Test 2: Single name that might be author
    test_author_search("Masaana Egede", language='da')
    
    # Test 3: Partial author name
    test_author_search("Kassaaluk", language='da')
    
    # Test 4: Multiple word query with potential author
    test_author_search("Nielsen Grønland", language='da')
